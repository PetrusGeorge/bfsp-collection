...
double MNEH::standard_deviation(const size_t job, Instance &instance, double average) {
    auto processing_time_matrix = [&instance](size_t i, size_t j) { return instance.p(i, j); };

    double sum = 0;

    for (size_t machine = 0; machine < instance.num_machines(); ++machine) {
        const double deviation = static_cast<double>(processing_time_matrix(job, machine)) - average;
...
std::vector<size_t> MNEH::priority_rule(const double alpha, Instance &instance) {
    ...
    for(size_t job = 0; job < instance.num_jobs(); job++) {
        averages.push_back(average(job, instance));
    }
    ...
    for (size_t job = 0; job < instance.num_jobs(); ++job) {
        const double priority_value =
            (alpha * averages[job]) + ((1 - alpha) * standard_deviation(job, instance, averages[job]));
