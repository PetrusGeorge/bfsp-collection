...
Solution GRASP_NEH::GRASP() {
    ...
    for (size_t i = 1; i < m_instance.num_jobs(); i++) {
        std::vector<std::vector<size_t>> departure_times = core::calculate_departure_times(m_instance, s.sequence);
        ...
        for (size_t j = 0; j < m_instance.num_jobs(); j++) {
            ...
            auto dp = core::calculate_new_departure_time(m_instance, departure_times, jobs[j]);
            c[j] = core::calculate_sigma(m_instance, departure_times, dp, jobs[j]);
