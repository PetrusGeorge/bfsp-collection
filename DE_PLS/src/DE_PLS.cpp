...
DE_PLS::DE_PLS(Instance instance, Parameters params) : m_instance(std::move(instance)), m_params(std::move(params)) {
...
void DE_PLS::initialize_population() {
    ...
    for (size_t i = 0; i < m_params.x(); i++) {
        const Solution pi = pf_neh.solve(m_params.delta());
...
void DE_PLS::perturbation(Solution &s) {
    ...
    for (size_t i = 0; i < s.ps; i++) {
        ...
        jobs.push_back(s.sequence[shift]);
...
void DE_PLS::desconstruct_construct(Solution &s) {
    ...
    helper.second_step(jobs, s);
}

size_t DE_PLS::find_best_solution() {
    size_t best = 0;
