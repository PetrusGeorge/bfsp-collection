...
Solution GRASP::solve(double beta) {
  ...
  std::vector<std::vector<size_t>> d(
      1, std::vector<size_t>(m, 0)); // departure time matrix

  std::vector<size_t> seq =
      core::stpt_sort(m_instance); // sorting using the stpt rule
  ...
  std::vector<bool> b_s(
      n,
      false); // vector bool to represent the jobs scheduled (true = scheduled)
  b_s[seq[0]] = true;
  ...
  for (size_t i = 1; i < n - 1; i++) {
    ...
    for (size_t j = 1; j < n; j++) {
      ...
      // calculating the departure time of the job j that will be possibly
      // inserted in the solution
      std::vector<size_t> new_departure_time =
          core::calculate_new_departure_time(m_instance, d, pi[i]);

      // computing the sum of the idle and blocking time of the job j
      sigma[sigma.size() - 1] =
          core::calculate_sigma(m_instance, d, new_departure_time, pi[i], i);
