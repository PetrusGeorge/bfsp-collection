...
Solution NEH::solve(std::vector<size_t> phi) {
  ...
  while (!phi.empty()) {
    set_taillard_matrices(s.sequence, phi.front());
...
void NEH::set_taillard_matrices(const std::vector<size_t> &sequence, size_t k) {
  ...
  m_q.emplace_back(m_instance.num_machines(),
                   0); // Make it easier to implement find_best_insertion
  ...
  m_f = std::vector(sequence.size() + 1,
                    std::vector<size_t>(m_instance.num_machines()));
  m_f[0][0] = p(k, 0);
  ...
  for (size_t i = 1; i <= sequence.size(); i++) {
    ...
    m_f[i].back() = m_f[i][m_instance.num_machines() - 2] +
                    p(k, m_instance.num_machines() - 1);
  }
}

std::vector<std::vector<size_t>>
NEH::calculate_departure_times(const std::vector<size_t> &sequence) {
  auto departure_times = std::vector(
      sequence.size(), std::vector<size_t>(m_instance.num_machines()));
  ...
  for (size_t i = 1; i < sequence.size(); i++) {
    ...
    departure_times[i][0] = std::max(departure_times[i - 1][0] + p(node, 0),
                                     departure_times[i - 1][1]);
    for (size_t j = 1; j < m_instance.num_machines() - 1; j++) {

      const size_t current_finish_time = departure_times[i][j - 1] + p(node, j);

      departure_times[i][j] =
          std::max(current_finish_time, departure_times[i - 1][j + 1]);
    }
    departure_times[i].back() =
        departure_times[i][m_instance.num_machines() - 2] +
        p(node, m_instance.num_machines() - 1);
  }
...
std::vector<std::vector<size_t>>
NEH::calculate_tail(const std::vector<size_t> &sequence) {

  auto tail = std::vector(sequence.size(),
                          std::vector<size_t>(m_instance.num_machines()));
  ...
  for (long i = ((long)sequence.size()) - 2; i >= 0; i--) {
    const size_t node = sequence[i];
    tail[i].back() =
        std::max(tail[i + 1].back() + p(node, m_instance.num_machines() - 1),
