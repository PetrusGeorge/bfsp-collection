...
Solution PF::solve(const Instance &instance) {
  ...
  for (size_t i = 1; i < n; ++i) {
    ...
    std::vector<std::vector<size_t>> d_current =
        core::calculate_departure_times(instance, new_seq);
    ...
    for (const size_t candidate : unscheduled) {
      ...
      for (size_t k = 1; k <= m; ++k) {
        sigma +=
            d_new[k] - (d_current.back()[k] + instance.p(candidate, k - 1));
      }
      if (sigma < best_sigma) {
        best_sigma = sigma;
        best_job = candidate;
    ...
    unscheduled.erase(
        std::remove(unscheduled.begin(), unscheduled.end(), best_job),
        unscheduled.end());
  ...
  std::vector<std::vector<size_t>> d_final =
      core::calculate_departure_times(instance, new_seq);
