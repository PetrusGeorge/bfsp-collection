...
Solution GraspNeh::solve(int x, int delta, Instance &instance) {
  ...
  for (int h = 0; h < x && h < static_cast<int>(sorted_jobs.size()); h++) {
    ...
    const size_t start_index =
        (delta > static_cast<int>(cand_sol.sequence.size()))
            ? 0
            : cand_sol.sequence.size() - delta;
    ...
    for (size_t pos = start_index; pos < cand_sol.sequence.size(); pos++) {
      ...
      for (size_t j = 0; j <= temp_seq.size(); j++) {
        ...
        auto d_candidate =
            core::calculate_departure_times(instance, candidate_seq);
        const size_t candidate_cmax =
            d_candidate
                .back()[m]; // makespan é o último valor da última máquina
