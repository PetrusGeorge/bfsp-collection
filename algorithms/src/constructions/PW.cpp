...
std::vector<double>
PW::calculate_avg_processing_time(size_t candidate_job,
...
void PW::update_avg_processing_time(
    const size_t previous_job, const size_t next_job,
...
std::vector<double> PW::calculate_artificial_departure_time(
  ...
  artificial_departure_time[0] =
      std::max(((double)d[d.size() - 1][0]) + artificial_processing_times[0],
               (double)d[d.size() - 1][1]);

  for (size_t j = 1; j < m - 1; j++) {

    const double current_finish_time =
        artificial_departure_time[j - 1] + artificial_processing_times[j];

    artificial_departure_time[j] =
        std::max(current_finish_time, (double)d[d.size() - 1][j + 1]);
  }
...
double PW::calculate_chi(std::vector<size_t> &new_departure_time,
  ...
  for (size_t machine = 0; machine < m; machine++) {
    ...
    const double sum = (artificial_departure_time[machine] -
                        (double)new_departure_time[machine] -
...
double PW::calculate_f(std::vector<std::vector<size_t>> &d,
                       std::vector<size_t> &new_departure_time, double chi,
                       size_t job, size_t k) {

  const size_t n = m_instance.num_jobs(); // number of jobs
  const size_t sigma =
      core::calculate_sigma(m_instance, d, new_departure_time, job, k);
...
Solution PW::solve() {
  ...
  std::vector<size_t> unscheduled(
      n); // list of unscheduled jobs (initially 0, 1, ..., n)
  std::iota(unscheduled.begin(), unscheduled.end(), 0);

  std::vector<double>
      artificial_processing_times; // processing time of the artificial job v
  std::vector<double>
      artificial_departure_time; // departure time of the artificial job v
  std::vector<size_t>
      new_departure_time; // hipotetical departure time of the job j

  size_t best_i =
      std::numeric_limits<size_t>::max(); // store the index of the variable
                                          // with smallest f
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    artificial_departure_time = calculate_artificial_departure_time(
        d_current, artificial_processing_times);

    chi = calculate_chi(d_current[0], artificial_departure_time,
    ...
    update_avg_processing_time(i, i + 1, unscheduled.size(),
                               artificial_processing_times);
  }
  ...
  for (size_t k = 1; k <= n - 2; k++) {
    ...
    artificial_processing_times =
        calculate_avg_processing_time(unscheduled[0], unscheduled);
    for (size_t i = 0; i < unscheduled.size(); i++) {

      artificial_departure_time = calculate_artificial_departure_time(
          d_current, artificial_processing_times);
      new_departure_time = core::calculate_new_departure_time(
          m_instance, d_current, unscheduled[i]);

      chi = calculate_chi(new_departure_time, artificial_departure_time,
                          artificial_processing_times);
      ...
      update_avg_processing_time(unscheduled[i], unscheduled[i + 1],
                                 unscheduled.size(),
                                 artificial_processing_times);
    }
