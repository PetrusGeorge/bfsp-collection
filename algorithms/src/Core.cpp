...
std::vector<size_t> core::stpt_sort(const Instance &instance) {
  ...
  std::sort(seq.begin(), seq.end(), [optzado, instance](size_t a, size_t b) {
...
size_t core::calculate_sigma(const Instance &instance,
  ...
  for (size_t machine = 0; machine < m; machine++) {
    ...
    if (k == 0) {
      ...
      sum = (new_departure_time[machine] - d[d.size() - 1][machine] -
             p(job, machine));
    }
...
std::vector<size_t>
core::calculate_new_departure_time(const Instance &instance,
                                   std::vector<std::vector<size_t>> &d,
                                   size_t node) {
