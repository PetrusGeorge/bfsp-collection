...
class Instance {
public:
  // The constructor of this class can throw in case it can't read the isntance
  // properly
  ...
  const std::vector<size_t> &processing_times_sum() const {
    return m_processing_times_sum;
  }

  // Main instance matrix
  long p(size_t i, size_t j) const { return m_matrix[i][j]; }
  // Reverse matrix
  long rp(size_t i, size_t j) const {
