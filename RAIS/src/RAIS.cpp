...
RAIS::RAIS(Instance instance, double alp, size_t threshold, size_t gt, size_t n) : m_instance(std::move(instance)), alpha(alp), d_threshold(threshold), Gt(gt), nc(n) {
...
void RAIS::pop_affinity_calculation(std::vector<std::pair<Solution, double>> &pop) { 

  for(size_t i = 0; i < pop.size(); i++) pop[i].second = affinity_calculation(pop[i].first.cost);
...
std::vector<std::pair<Solution, double>> RAIS::clone_antibodies(std::vector<std::pair<Solution, double>> &pop) {
...
void RAIS::supression(std::vector<std::pair<Solution, double>> &pop, std::vector<std::pair<Solution, double>> &clones) {
...
void RAIS::update(std::vector<std::pair<Solution, double>> &pop, std::vector<std::pair<Solution, double>> &clones) {
