...
core::calculate_departure_times(Instance &instance,
  ...
  for (size_t i = 1; i < sequence.size(); i++) {
    ...
    departure_times[i].back() =
        departure_times[i][instance.num_machines() - 2] +
...
std::vector<std::vector<size_t>>
core::calculate_tail(Instance &instance, const std::vector<size_t> &sequence) {

  auto tail = std::vector(sequence.size(),
                          std::vector<size_t>(instance.num_machines()));
  ...
  for (long i = ((long)sequence.size()) - 2; i >= 0; i--) {
    const size_t node = sequence[i];
    tail[i].back() =
        std::max(tail[i + 1].back() + p(node, instance.num_machines() - 1),
...
std::vector<size_t> core::stpt_sort(Instance &instance) {
  ...
  std::sort(seq.begin(), seq.end(), [&instance](size_t a, size_t b) {
    return instance.processing_times_sum()[a] <
           instance.processing_times_sum()[b];
  });

  return seq;
}

size_t core::calculate_sigma(Instance &instance,
  ...
  for (size_t machine = 0; machine < m; machine++) {
    ...
    if (k == 0) {
      ...
      sigma += (new_departure_time[machine] - d[d.size() - 1][machine] -
                p(job, machine));
    }
  }
...
std::vector<size_t> core::calculate_new_departure_time(
    Instance &instance, std::vector<std::vector<size_t>> &d, size_t node) {
