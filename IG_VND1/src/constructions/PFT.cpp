...
PFT::PFT(Instance &instance) : m_instance(instance) {}

//---------------------------------------------------------
// PFT heuristic implementation.
...
Solution PFT::solve() {
    ...
    std::vector<size_t> initial = core::initial_sort(m_instance);

    pft_insertion_phase(sol, initial.front());

    return sol;
}

void PFT::pft_insertion_phase(Solution &s, size_t first_job) {
